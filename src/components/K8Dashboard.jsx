...
export default function K8Dashboard() {
  ...
  useEffect(() => {
    ...
    const metricsToFetch = ['prof_pct_mth_all', 'prof_pct_ela_all'];
  ...
  useEffect(() => {
    ...
    // High School removed from K-8 Dashboard
    ...
    Object.keys(districtAgg).forEach(dist => {
      const getVal = (mName) => {
        ...
        let rawVal = obj.sumWeighted / obj.totalWeight;
        rawVal *= 100;
    ...
    if (selectedDistrict) {
      setSelectedDistrict(prev => ({
        id: prev.id,
        ...(finalMetrics[prev.id] || { math: null, reading: null })
      }));
    }
  ...
  geojson.features.forEach(feature => {
    ...
    if (data) {
      ...
      textLabels.push(`District ${distId}<br>Math: ${mStr}<br>Reading: ${rStr}`);
    }
  });
    ...
    <div style={{ display: 'flex', height: '100vh', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ flex: 1, position: 'relative' }}>
        ...
        <div style={{ position: 'absolute', top: 20, left: 20, zIndex: 10, backgroundColor: 'white', padding: '15px 25px', borderRadius: '8px', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}>
          <h1 style={{ margin: 0, fontSize: '1.5em', color: '#333' }}>K-8 Grades</h1>
